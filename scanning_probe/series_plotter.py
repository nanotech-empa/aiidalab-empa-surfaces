...
class SeriesPlotter():
    ...
    def add_series_collection(self, general_info, series_info, series_data):
        ...
        self.figure_xy_ratio = (np.max(x_arr)-np.min(x_arr)) / (np.max(y_arr)-np.min(y_arr))
    ...
    def add_selection_row(self):
        ...
        drop_full_series = ipw.Dropdown(description="series", options=sorted(self.series.keys(), reverse=True),
        ...
        for i in index_list:
            ...
            with plot_out:
                ...
                for i_ser in range(num_series):
                    ...
                    make_plot(fig, ax, data[i, :, :], center0=sym_check, vmin=vmin, vmax=vmax,
