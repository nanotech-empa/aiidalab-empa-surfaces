...
def get_types(frame, thr):  ## Piero Gasparotto
    ...
    if len(possible_mol_atoms) > 0:
        cov_radii = [
            covalent_radii[a.number] for a in frame[possible_mol_atoms]
        ]  # adatoms that have a neigh adatom are in a mol
...
def analyze(atoms):
    ...
    vacuum_x = (
        np.max(atoms.positions[:, 0]) - np.min(atoms.positions[:, 0]) + 4
        < atoms.cell[0][0]
    )
    vacuum_y = (
        np.max(atoms.positions[:, 1]) - np.min(atoms.positions[:, 1]) + 4
        < atoms.cell[1][1]
    )
    vacuum_z = (
        np.max(atoms.positions[:, 2]) - np.min(atoms.positions[:, 2]) + 4
        < atoms.cell[2][2]
    )
    all_elements = (
