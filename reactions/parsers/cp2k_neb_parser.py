...
class Cp2kNebParser(Parser):
    ...
    def _parse_stdout(self):
        ...
        energy_str_list = re.findall(r'ENERGIES \[au\] = (.*?)BAND', output_string, re.DOTALL)
        ...
        dist_str_list = re.findall(r'DISTANCES REP = (.*?)ENERGIES', output_string, re.DOTALL)
        distances_list = [np.array(['0.0']+ds.split(),dtype=float) for ds in dist_str_list]
    ...
    def _parse_trajectory(self):
        ...
        for i_rep, rep_coord_lines in enumerate(replica_coord_line_sets):
            ...
            self.out("opt_replica_%d" % i_rep, StructureData(ase=ase_atoms))
