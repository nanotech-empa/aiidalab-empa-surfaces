...
class DFTDetails(ipw.VBox):        
    def __init__(self,
        ...
        self.ot_switch = ipw.ToggleButtons(options=['OT','DIAG'],
                                       description='OT',
                                       value='OT',
                                       style=style, layout=layout)
        
        self.added_mos = ipw.IntText(value=10,
        ...
        ###GW SECTION
        self.gw_type = ipw.Dropdown(options=['GW','GW-IC','GW-LS'],
                                   description='GW type',
                                   value='GW',
                                   style=style, layout=layout)
        ...
        ###EPS_DEFAULT  1.0E-15    
        self.eps_default = ipw.IntSlider(value=-15,min=-30, 
                                        max=-12, step=1, description='EPS_DEFAULT 10^-',
                                        style=style, layout=layout
                                        )
        ...
        ###EPS_GRID 1.0E-12    
        self.eps_grid = ipw.IntSlider(value=-12,min=-30, 
        ...
        self.eps_pgf_orb_s = ipw.IntSlider(value=-12,min=-30, 
                                      max=-12, step=1, description='EPS_PGF_ORB_S 10^-',
                                      style=style, layout=layout
                                     )
        ...
        self.eps_filter_im_time = ipw.IntSlider(value=-12,min=-20, 
                                         max=-12, step=1, description='EPS_FILTER_IM_TIME 10^-',
                                         style=style, layout=layout
                                        )
...
##END GW
        ...
        def check_charge():
            with self.message_output:
                clear_output()
                total_charge=self.job_details['slab_analyzed']['total_charge'] +  self.charge.value
                if total_charge % 2 > 0 :
        ...
        self.independent_widget_jd = OrderedDict([                               
            ...
            ('gw_type'      , get_gw_values),  ##TO DO switching gw_type the old job_entries remain defined
...
##################SUPER        
    ...
    def create_gw_boxes(self):
        gw_type=self.gw_type.value
        self.gw_children.children = tuple([  self.gw_w_dict[gw_type][key] for key in self.gw_w_dict[gw_type].keys()  ])
