...
class SearchCompletedWidget(ipw.VBox):
    ...
    def preprocess_one(self, workcalc):
        ...
        if workcalc.exit_status == 401:
            raise (
                Exception(
                    "The workchain reached the maximum step number. Check and resubmit manually"
                )
            )
        ...
        # optimized structure
        calc = get_calc_by_label(
            workcalc, self.clabel
        )  # TODO deal with restarts, check final state
        opt_structure = calc.outputs.output_structure

        # initial structure:
        maybe = (
            workcalc.inputs.cp2k__file__input_xyz.get_incoming()
            .all_nodes()[0]
            .get_incoming()
            .all_nodes()
        )
        ...
        if len(res["slabatoms"]) > 0:
            ...
            if len(res["bottom_H"]) > 0:
                slab_formula += (
                    " saturated: " + ase_struct[res["bottom_H"]].get_chemical_formula()
                )
        ...
        workcalc.set_extra(
            "cell",
            calc.outputs.output_structure.get_ase().get_cell_lengths_and_angles(),
        )
        workcalc.set_extra(
            "volume", calc.outputs.output_structure.get_ase().get_volume()
        )
