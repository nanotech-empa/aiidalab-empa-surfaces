...
class DFTDetails(ipw.VBox):
    def __init__(self, workchain="SlabGeoOptWorkChain", structure_details={}, **kwargs):
        ...
        self.convergence.children = [
            ipw.VBox([self.convergence_dict[k] for k in self.convergence_dict.keys()])
        ]
        ...
        self.cell_spec.children = [
            ipw.VBox([self.cell_spec_dict[k] for k in self.cell_spec_dict.keys()])
        ]
        ...
        self.gw_type_btn = ipw.RadioButtons(
            description="GW type",
            options=["GW", "GW-LS", "GW-IC"],
            value="GW",
            disabled=False,
        )
        ###GROUP_SIZE 12
        ...
        ###EPS_DEFAULT  1.0E-15
        self.eps_default = ipw.FloatLogSlider(
            value=-15,
            min=-30,
            base=10,
            max=-12,
            step=1,
            description="EPS_DEFAULT",
            style=style,
            layout=layout,
        )
        ###EPS_FILTER  1.0E-12
        self.eps_filter = ipw.FloatLogSlider(
            value=-12,
            min=-20,
            base=10,
            max=-12,
            step=1,
            description="EPS_FILTER",
            style=style,
            layout=layout,
        )
        ###EPS_GRID 1.0E-12
        self.eps_grid = ipw.FloatLogSlider(
            value=-12,
            min=-30,
            base=10,
            max=-12,
            step=1,
            description="EPS_GRID",
            style=style,
            layout=layout,
        )
        ###EPS_PGF_ORB_S 1.0E-30
        self.eps_pgf_orb_s = ipw.FloatLogSlider(
            value=-12,
            min=-30,
            base=10,
            max=-12,
            step=1,
            description="EPS_PGF_ORB_S",
            style=style,
            layout=layout,
        )
        ...
        self.eps_schwarz = ipw.FloatLogSlider(
            value=-13,
            min=-20,
            base=10,
            max=-13,
            step=1,
            description="EPS_SCHWARZ",
            style=style,
            layout=layout,
        )

        ###EPS_FILTER_IM_TIME 1.0E-12 IC
        self.eps_filter_im_time = ipw.FloatLogSlider(
            value=-12,
            min=-20,
            base=10,
            max=-12,
            step=1,
            description="EPS_FILTER_IM_TIME",
            style=style,
            layout=layout,
        )
        ...
        self.gw_dict = {
            ...
            "GW-LS": {
                "group_size_3c": self.group_size_3c,
        ...
        def on_gw_select(c):
            ...
            self.gw.children = [
                ipw.VBox(
                    [
                        self.gw_dict[self.gw_type][k1]
                        for k1 in self.gw_dict[self.gw_type].keys()
                    ]
                )
            ]
        ...
        def on_dft_toggle(v):
            ...
            if "Slab" in self.structure_details["system_type"]:
                if self.calc_type.value == "Full DFT":
                    to_fix = [
                        i
                        for i in self.structure_details["bottom_H"]
                        + self.structure_details["slab_layers"][0]
                        + self.structure_details["slab_layers"][1]
                    ]
                else:
                    to_fix = (
                        self.structure_details["bottom_H"]
                        + self.structure_details["slabatoms"]
                    )
        ...
        self.all_widgets = {
            "fixed_atoms": {
                "display": self.fixed_display,
                "dict": "no",
                "retrieve": self.fixed_atoms,
            },
            "convergence": {
                "display": self.convergence,
                "dict": self.convergence_dict,
                "retrieve": self.convergence_dict,
            },
            "calc_type": {
                "display": self.calc_type,
                "dict": "no",
                "retrieve": self.calc_type,
            },
            "vdw_switch": {
                "display": self.vdw_switch,
                "dict": "no",
                "retrieve": self.vdw_switch,
            },
            "gw": {
                "display": self.gw_display,
                "dict": self.gw_dict,
                "retrieve": self.get_gw_values,
            },
            "cell": {
                "display": self.cell_spec,
                "dict": self.cell_spec_dict,
                "retrieve": self.cell_spec_dict,
            },
            "uks": {
                "display": self.uks,
                "dict": self.uks_dict,
                "retrieve": self.uks_dict,
            },
        }
        ...
        visualize_widgets = [
            self.all_widgets[k]["display"] for k in self.widgets_enabled
        ]
    ...
    def get_widget_values(self, the_dict):
        if (
            "Slab" in self.structure_details["system_type"]
            and self.calc_type.value != "Full DFT"
        ):
            the_dict["first_slab_atom"] = (
                min(
                    self.structure_details["bottom_H"]
                    + self.structure_details["slabatoms"]
                )
