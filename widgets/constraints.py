...
class OneConstraint(ipw.HBox):
    ...
    def _observe_constraint(self, change):
        if len(self.children) == 1 and "distance" in change["new"]:
            self.children += (OneColvar(cvtype="distance"),)
        elif len(self.children) == 1 and "angle" in change["new"]:
...
class ConstraintsWidget(ipw.VBox):
    ...
    def add_constraint(self, b=None):
        ...
        if (
            ...
            self.constraints.children[
                0
            ].constraint_widget.value = "fixed atoms " + mol_ids_range(to_fix)
    ...
    def return_dict(self):
        ...
        return {"constraints": constraints[:-1], "colvars": colvars[:-1]}
